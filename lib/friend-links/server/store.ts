...
export async function listFriendLinks(): Promise<FriendLink[]> {
  return db
    .select()
    .from(friendLinks)
    .orderBy(asc(friendLinks.sortOrder), asc(friendLinks.createdAt));
}

/** 按 ID 获取友链 */
export async function getFriendLinkById(id: string): Promise<FriendLink | null> {
  const rows = await db.select().from(friendLinks).where(eq(friendLinks.id, id)).limit(1);
  return rows[0] ?? null;
}
...
export async function updateFriendLink(
  ...
  const rows = await db.update(friendLinks).set(data).where(eq(friendLinks.id, id)).returning();
  return rows[0] ?? null;
}
