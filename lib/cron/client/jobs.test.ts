...
describe('isOrphanSystemJob', () => {
  it('标题 [blog: 且不匹配预设 → true', () => {
    expect(isOrphanSystemJob(makeJob({ title: '[blog:dead-key] 不存在的预设' }))).toBe(true);
  });
...
describe('availableActions', () => {
  ...
  it('普通任务 → toggle + 编辑/历史/删除', () => {
    const c = classifyJob(makeJob());
    expect(availableActions(makeJob(), c)).toEqual(['toggle', 'edit', 'history', 'delete']);
  });
});
