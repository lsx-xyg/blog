/**
 * 管理首页统计卡片 + 快捷入口顺序：默认顺序 + 归一化（服务端与 API 共用）
 */
...
export function normalizeOrder(order: unknown, keys: readonly string[]): string[] {
  const input = Array.isArray(order) ? (order as unknown[]) : [];
  const seen = new Set<string>();
  const normalized: string[] = [];
  for (const item of input) {
    if (typeof item === 'string' && (keys as readonly string[]).includes(item) && !seen.has(item)) {
      seen.add(item);
