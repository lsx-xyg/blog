 ...
 */
export function createShikiRehypePlugin(highlighter: HighlighterCore, theme: string): BytemdPlugin {
  return {
    rehype: (p) =>
      p.use(() => (tree: Root) => {
        visit(tree, 'element', (node: Element) => {
          ...
          const lang =
            className.find((c) => c.startsWith('language-'))?.replace('language-', '') || 'text';
