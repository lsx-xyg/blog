...
import { buildTourSteps, pickSelector } from '../shared/tour';
...
describe('buildTourSteps', () => {
  it('nextRoute 拼接：adminPath + 步骤路由 → /dashboard/settings', () => {
    const steps = buildTourSteps(makeGuide([makeStep({ nextRoute: '/settings' })]), 'dashboard', {
      isHit: hits(new Set()),
    });
  ...
  it('side 取 placement，默认 bottom', () => {
    ...
    const top = buildTourSteps(makeGuide([makeStep({ placement: 'top' })]), 'd', {
      isHit: hits(new Set()),
    });
  ...
  it('步骤配置了动态选择器且 DOM 命中 → 用动态选择器', () => {
    const guide = makeGuide([
      makeStep({
        ...
        selectorMeta: { source: 'class', generatedAt: '2026-09-17T00:00:00.000Z' },
      }),
    ]);
    const steps = buildTourSteps(guide, 'd', {
