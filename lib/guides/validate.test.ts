...
describe("isConditionValid", () => {
  it("click_count 空锚点无效", () => {
    expect(isConditionValid({ field: "click_count.", op: GuideConditionOp.GTE, value: 1 })).toBe(false);
    expect(isConditionValid({ field: "click_count.cron-save", op: GuideConditionOp.GTE, value: 1 })).toBe(true);
  });
  ...
  it("非 exists 条件需有 value", () => {
    ...
    // exists 不需要 value
    expect(isConditionValid({ field: "user_age_days", op: GuideConditionOp.EXISTS, value: undefined })).toBe(true);
  });
});
