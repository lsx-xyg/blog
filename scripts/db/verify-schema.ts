...
async function main() {
  ...
  try {
    ...
    const types = await sql`select t.typname from pg_type t join pg_namespace n on n.oid = t.typnamespace where n.nspname = 'public' and t.typtype = 'e' order by t.typname`;
    const enumVals = await sql`select t.typname, e.enumlabel from pg_type t join pg_enum e on t.oid = e.enumtypid join pg_namespace n on n.oid = t.typnamespace where n.nspname = 'public' and t.typtype = 'e' order by t.typname, e.enumsortorder`;
    ...
    console.log("─".repeat(50));
    console.log(`user.isAdmin 列: ${isAdmin.length ? `✅ 存在 (${isAdmin[0].data_type})` : "❌ 缺失"}`);
  } finally {
    await sql.end();
  }
}
