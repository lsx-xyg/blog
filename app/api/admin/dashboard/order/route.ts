...
export async function GET(req: Request) {
  ...
  try {
    ...
    const normalize = scope === 'quick' ? normalizeQuickOrder : normalizeCardOrder;
    ...
    return NextResponse.json({ error: '读取顺序失败', detail: String(error) }, { status: 500 });
  }
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (!Array.isArray(body.order) || body.order.length === 0) {
      return NextResponse.json({ error: 'order 必须是 key 数组' }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: '保存顺序失败', detail: String(error) }, { status: 500 });
  }
}
