...
import { FileText, Trash2, Pencil, CheckSquare, Square } from 'lucide-react';
      ...
      >
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="w-28 px-4 py-3 text-right text-sm font-medium text-muted-foreground">
                    操作
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3">
                      {Array.isArray(p.tags) && p.tags.length > 0 ? (
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          {p.tags.length > 3 && (
                            <span className="text-[11px] text-fg-faint">+{p.tags.length - 3}</span>
                          )}
                        </div>
                      ) : (
          ...
          <div className="md:hidden overflow-hidden rounded-xl border border-border bg-card">
            <ul key={`${statusFilter}-${search}`} className="divide-y divide-border">
              {filteredPosts.map((p, i) => (
                <li
                  key={p.id}
                  style={{ animationDelay: `${Math.min(i * 30, 300)}ms` }}
